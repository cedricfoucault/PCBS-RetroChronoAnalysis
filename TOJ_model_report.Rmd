---
title: "Modélisation du Temporal Order Judgement dans l'effet de rétro-perception"
...
On prédit différentes fonctions la condition (avec-cue ou no-cue), le type d'essai, l'hypothèse que l'on admet, et l'intervalle de temps entre la cible et la cue `soa.toCue`.

On réutilise les paramètres et la fonction helper ci-dessous pour visualiser les fonctions psychométriques au fur et à mesure :
``` {r helpers}
data.soa.toCue <- 300
...
plot.function.retro <- function(model) {
  p.function.retro <- function(soa.toBeep) { p.beforeBeep.retro(soa.toBeep, data.soa.toCue, model) }
  title <- sprintf("Retro - %s", model$name, data.soa.toCue)
  subtitle <- sprintf("SOA[Target-Cue] = %d ms", data.soa.toCue)
  plot.function(p.function.retro, model$color, title, subtitle)
}
```

``` {r.component.retro.plot}
plot.function.retro(model.CA)
plot.function.retro(model.TM)
```

## Fonctions psychométriques résultantes : Mixture Model

On modélise les fonctions psychométriques résultantes par une distribution de mélange (mixture model) résultant de la combinaison des composantes décrites ci-dessus, pondérée par les probabilités de chaque type d’essai. Pour calculer ces probabilités, on se base sur les résultats de l'expérience de rétroperception déjà menée par Thibault, Sergent et al. (2016).

En condition no-cue, le seuil de contraste auquel on se place donne une probabilité de 0.55 de voir consciemment la cible, ce qui nous donne directement les probabilités qu'un essai soit de type *guess* ou de type *seen* :
``` {r p.trials.nocue}
p.seen.nocue <- 0.55 # data from Thibault, Sergent et al. (2016)
p.trials.nocue <- list(guess = 1 - p.seen.nocue, seen = p.seen.nocue)
```

En condition avec-cue, la probabilité de voir consciemment la cible dépend du SOA entre target et cue. On interpole les données obtenues expérimentalement pour obtenir la probabilité qu'un essai `p.guess` soit de type *guess*, et on en déduit les autres probabilités comme suit :
``` {r p.trials}
...
p.trials <- function(soa.toCue) {
  ...
  if (soa.toCue >= 0) {
    ## lorsque la cue apparaît après la cible, la probabilité de voir la cible directement est la même qu'en condition nocue car la cue ne peut avoir qu'un effet rétrospectif
    p.seen <- p.seen.nocue
...
On obtient ainsi les fonctions psychométriques résultantes…

* Pour la condition no-cue :
...
plot.function.mixture <- function(model) {
  p.function.mixture <- function(soa.toBeep) { p.beforeBeep.mixture(soa.toBeep, data.soa.toCue, model) }
...
## Comparaison avec-cue vs. no-cue en fonction du SOA cue-target selon l'hypothèse admise
...
plot.mixtures <- function(model, input.soa.toCue = data.soa.toCue) {
  title <- sprintf("%s Model", model$name)
  subtitle <- sprintf("SOA[Target-Cue] = %d ms", input.soa.toCue)
  color <- model$color
  lty <- "dashed"
  plot(data.soa.toBeep,
       p.beforeBeep.mixture.nocue(data.soa.toBeep),
       type="l", col="black", lty="dotted",
       xlab="SOA[Target-Beep] (ms)", ylab="P(\"T < B\")",
       ylim=c(0,1),
       main=title)
  lines(data.soa.toBeep,
        p.beforeBeep.mixture(data.soa.toBeep, soa.toCue = input.soa.toCue, model),
        col=color, lty=lty)
  mtext(subtitle)
...
sliderInput("soa.toCue.comparison", "SOA[Target-Cue] (ms)", -100, 400, data.soa.toCue, step = 25, round = TRUE)

renderPlot({ plot.mixtures(model.CA, input$soa.toCue.comparison) })
renderPlot({ plot.mixtures(model.TM, input$soa.toCue.comparison) })
```

## Analyse

On voit ci-dessus que, sous l'hypothèse CA, pour les cue avec SOA[target-cue] positifs, on observe sur la fonction psychométrique les différences suivantes entre la condition "avec-cue" et celle de la condition "no-cue" :
...
Sous l'hypothèse TM, on observe aucune de ces différences.

Ainsi, nous pourrions tester lequel des deux modèles décrit le mieux nos données en manipulant expérimentalement le SOA[target-cue], en calculant les différences mentionnées et en observant leur évolution en fonction du SOA.

## Conclusion

Ce projet a été l'occasion pour moi d'apprendre R et R Markdown. Je ne les avais jamais utilisés auparavant.

L'écriture du rapport m'a forcé à rendre mon code plus lisible et plus simple, à utiliser une approche plus basée sur des structures fonctionnelles que des structures impératives et à minimiser les dépendences (pas de variable globale, injection de dépendences dans les fonctions).

Cela m'a aussi appris à écrire un rapport d'analyse de données partageable qui combine code, visualisations et explications. Avant le cours d'Info 2, je n'avais jamais vu de tels documents scientifiques et je ne savais pas qu'il existait des outils comme R Markdown pour les produire aussi facilement. Je trouve cela "assez génial" ! :-)
