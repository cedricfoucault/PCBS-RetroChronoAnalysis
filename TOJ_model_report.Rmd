...
```
Pour la `jnd`, dans les deux hypothèses, on prédit que le déclin de la trace sensorielle récupérée à l’apparition de l’indice cause une diminution de la précision et donc du `jnd` par rapport à un essai seen, diminution d’autant plus grande que le `soa.toCue` est grand.
``` {r jnd.toBeep.retro}
...
sliderInput("soa.toCue.comparison", "SOA[Target-Cue] (ms)", -100, 400, soa.toCue.default, step = 25, round = TRUE)
