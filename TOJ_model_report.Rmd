...
for (model in models) {
  p.function.mixture <- function(soa.toBeep) {
      p.beforeBeep.mixture(soa.toBeep, data.soa.toCue, model$name, include.guesses)
  }
...
plot.mixtures <- function(model, color, input.soa.toCue = data.soa.toCue) {
  plot(data.soa.toBeep,
       p.beforeBeep.mixture.nocue(data.soa.toBeep, TRUE),
       type="l", col="black", lty="dotted",
       xlab="Target-to-Beep SOA (ms)", ylab="P(\"T < B\")",
       ylim=c(0,1),
       main=sprintf("%s Model", model))
  lines(data.soa.toBeep,
        p.beforeBeep.mixture(data.soa.toBeep, soa.toCue = input.soa.toCue, model, include.guesses),
