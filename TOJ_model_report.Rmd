...
plot.function.retro <- function(model) {
  ...
  title <- sprintf("Retro - %s - SOA(target-cue): %d ms", model$name, data.soa.toCue)
  plot.function(p.function.retro, model$color, title)
}
```
...
plot.function.mixture <- function(model) {
  ...
  plot.function(p.function.mixture, model$color, title)
}
```
...
plot.mixtures <- function(model, input.soa.toCue = data.soa.toCue) {
  ...
  legend("topleft", inset=.05, lwd=2,
         c("no cue", sprintf("with cue : %dms", input.soa.toCue)),
         col=c("black", color),
         lty=c("dotted", lty))
}
```
